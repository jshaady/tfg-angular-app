...
export class MeetService {
  ...
  private meets: IMeet[] | undefined;
  private meet: IMeet | undefined;
  private isSaving: Boolean = false;
  private createErrors: any = [];

  private dataSource: MatTableDataSource<IMeet> | undefined;
  private meetsPageOptions: any = {
    previousPageIndex: 0,
    pageIndex: 0,
    pageSize: 25,
    length: 0,
  ...
  searchMeets(): void {
    ...
    this.http.get<IMeet[]>(this.BaseURL + "/list", { params }).subscribe(
      (data: any) => {
  ...
  joinMeet(): void {
    this.isSaving = true;
    this.http
      .post<ISnackBar>(this.BaseURL + "/join", {
        idMeet: this.meet!.id.toString(),
      })
      .subscribe(
        (data: ISnackBar) => {
          this.makeSnackBar(data.success, "success");
  ...
  private checkCreateFormErrors(errors: any): void {
    ...
    if (errors.meetNameMaxLength)
      this.setCreateErrors(errors.meetNameMaxLength);
    if (errors.meetNameMinLength)
      this.setCreateErrors(errors.meetNameMinLength);
    if (errors.meetSportNull) this.setCreateErrors(errors.meetSportNull);
    if (errors.meetSportMaxLength)
      this.setCreateErrors(errors.meetSportMaxLength);
    if (errors.meetSportMinLength)
      this.setCreateErrors(errors.meetSportMinLength);
    if (errors.meetLocationNull) this.setCreateErrors(errors.meetLocationNull);
    if (errors.meetLocationMaxLength)
      this.setCreateErrors(errors.meetLocationMaxLength);
    if (errors.meetLocationMinLength)
      this.setCreateErrors(errors.meetLocationMinLength);
    if (errors.meetDescriptionNull)
      this.setCreateErrors(errors.meetDescriptionNull);
    if (errors.meetDescriptionMaxLength)
      this.setCreateErrors(errors.meetDescriptionMaxLength);
    if (errors.meetDescriptionMinLength)
      this.setCreateErrors(errors.meetDescriptionMinLength);
