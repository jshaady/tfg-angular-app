...
export class ChampionshipService {
  ...
  private championships: IChampionship[] | undefined;
  private clasification: IClasificationTeam[] | undefined;

  private joined: boolean = false;
  private notTeam: boolean = false;
  private joinResponse: any;
  private matches: IMatch[] | undefined;
  private bracketsMatches: IMatch[] | undefined;
  private matchDates: string[] = [];

  private page: number = 1;
  private paginate: IPaginate | null = null;
  private searchChampionshipType: string | undefined;
  private searchChampionshipName: string | null = null;
  private searchChampionshipLocation: string | null = null;
  private searchChampionshipSport: string = "all";

  private errorsJoinChampionship: string | null = null;
  private errorSetResult: string | null = null;
  private createErrors: any = [];

  private navLinksChampionship: any = [];

  private matchesDisplayedColumns: string[] = [
    "date",
    "team1",
    "result",
    "team2",
  ...
  private matchesTournamentDisplayedColumns: string[] = [
    "date",
    "team1",
    "result",
    "team2",
  ...
  private clasificationDisplayedColumns: string[] = [
    "team",
    "points",
    "won",
    "drawn",
  ...
  private leagueEventsDataSource: MatTableDataSource<IMatch> | undefined;
  private leagueEventsPageOptions: any = {
    previousPageIndex: 0,
    pageIndex: 0,
    pageSize: 25,
    length: 0,
  ...
  private tournamentEventsDataSource: MatTableDataSource<IMatch> | undefined;
  private tournamentEventsPageOptions: any = {
    previousPageIndex: 0,
    pageIndex: 0,
    pageSize: 25,
    length: 0,
  ...
  private matchesDataSource: MatTableDataSource<IMatch> | undefined;
  private matchesPageOptions: any = {
    previousPageIndex: 0,
    pageIndex: 0,
    pageSize: 25,
    length: 0,
  ...
  private teamnameSearched: string | null = null;
  ...
  private semifinalArray: any = [];
  private quarterfinalArray: any = [];
  private roundOf16Array: any = [];
  private roundOf32Array: any = [];
  ...
  searchChampionships(): void {
    ...
    const params = new HttpParams()
      .set("type", this.searchChampionshipType!)
  ...
  joinChampionship(body: any, onAdd: any): void {
    ...
    this.http.post<ISnackBar>(this.BaseURL + "/join", body).subscribe(
      (data: ISnackBar) => {
        this.isSaving = false;
        this.makeSnackBar(data.success, "success");
        this.getChampionshipByIdAndType(
          this.championship!.id.toString(),
          this.getChampionship()!.type
        );
        onAdd.emit();
      },
      (error: any) => {
        ...
        if (error instanceof HttpErrorResponse) {
          if (error.status === 401) {
            ...
            this.makeSnackBar(error.error.error, "error");
            this.router.navigate([
              "/" + this.getChampionship()!.type.toLowerCase(),
              this.getChampionship()!.id,
            ]);
          } else {
            onAdd.emit();
            this.checkHttpErrors(error);
  ...
  leftChampionship(idChampionship: string): void {
    ...
    this.http.delete<ISnackBar>(this.BaseURL + "/left", { params }).subscribe(
      (data: ISnackBar) => {
        this.makeSnackBar(data.success, "success");
        this.getChampionshipByIdAndType(
          this.getChampionship()!.id.toString(),
          this.getChampionship()!.type
        );
        this.router.navigate([
          "/" + this.getChampionship()!.type.toLowerCase(),
  ...
  searchChampionshipMatches(): void {
    const params = new HttpParams()
      .set("idChampionship", this.getChampionship()!.id.toString())
  ...
  searchChampionshipBracketsMatches(): void {
    ...
    this.http
      .get<IMatch[]>(this.BaseURL + "/brackets/matches", { params })
      .subscribe(
        (data: IMatch[]) => {
  ...
  deleteDate(query: any, add: any) {
    const params = new HttpParams()
      .set("idChampionship", query.idChampionship)
      .append("team1", query.team1)
      .append("team2", query.team2)
      .append("date", query.date);
    this.http
      .delete<ISnackBar>(this.BaseURL + "/delete/date", { params })
      .subscribe(
        (data: ISnackBar) => {
          this.isSaving = false;
          this.makeSnackBar(data.success, "success");
          this.getMatchDatesReq(
            query.idChampionship,
            query.team1,
            query.team2,
            add
          );
  ...
  generateMatches(): void {
    ...
    this.http
      .post<ISnackBar>(this.BaseURL + "/generate/matches", body)
      .subscribe(
        (data: ISnackBar) => {
          this.isSaving = false;
          this.makeSnackBar(data.success, "success");
        ...
        (error: any) => {
          ...
          if (error instanceof HttpErrorResponse) {
            if (
              error.status === 409 &&
              this.getChampionship()!.type == "League"
            ) {
              this.makeSnackBar(error.error.error, "error");
              this.router.navigate(["/league", this.getChampionship()!.id]);
            } else if (
              error.status === 409 &&
              this.getChampionship()!.type == "Tournament"
            ) {
              this.makeSnackBar(error.error.error, "error");
  ...
  getLeagueClasification(): void {
    ...
    this.http
      .get<IClasificationTeam[]>(this.BaseURL + "/clasification", { params })
      .subscribe(
        (data: IClasificationTeam[]) => {
          this.clasification = data;
          if (data.length > 0) this.clasification = data;
          else this.clasification = [];
        },
        (error: any) => {
          if (error instanceof HttpErrorResponse) {
            this.checkHttpErrors(error);
  ...
  getChampionshipByIdAndType(idChampionship: string, type: string): void {
    const params = new HttpParams()
      .set("idChampionship", idChampionship)
      .append("type", type);
      ...
      (data: IChampionship) => {
        ...
        if (type == "Tournament") {
          this.navLinksChampionship = [
            {
              label: "DESCRIPTION",
              path: "/tournament/" + this.championship.id.toString(),
            },
            {
              label: "BRACKETS",
              path:
                "/tournament/" + this.championship.id.toString() + "/brackets",
            },
            {
              label: "MATCHES",
              path:
                "/tournament/" + this.championship.id.toString() + "/matches",
            },
          ];
        }
        if (type == "League") {
          this.navLinksChampionship = [
            {
              label: "DESCRIPTION",
              path: "/league/" + this.championship.id.toString(),
            },
            {
              label: "CLASIFICATION",
              path:
                "/league/" + this.championship.id.toString() + "/clasification",
            },
            {
              label: "MATCHES",
              path: "/league/" + this.championship.id.toString() + "/matches",
            },
          ];
        }
  ...
  generateNextPhase(): void {
    ...
    this.http
      .post<ISnackBar>(this.BaseURL + "/generate/next/phase", body)
      .subscribe(
        (data: ISnackBar) => {
          this.isSaving = false;
          this.makeSnackBar(data.success, "success");
  ...
  searchUserNextMatches(type: string): void {
    const params = new HttpParams()
      ...
      .append(
        "pageIndex",
        type === "League"
          ? this.getLeagueEventsPageOptions().pageIndex
          : type === "Tournament"
          ? this.getTournamentEventsPageOptions().pageIndex
          : null
      )
      .append(
        "pageSize",
        type === "League"
          ? this.getLeagueEventsPageOptions().pageSize
          : type === "Tournament"
          ? this.getTournamentEventsPageOptions().pageSize
          : null
      );
    this.http
      .get<IMatch[]>(this.BaseURL + "/user/next/matches", { params })
      .subscribe(
        (data: any) => {
          if (type === "League") {
            this.leagueEventsDataSource = new MatTableDataSource<IMatch>(
              data.matches
            );
            this.leagueEventsPageOptions.length = data.pager.totalItems;
          } else if (type === "Tournament") {
            this.tournamentEventsDataSource = new MatTableDataSource<IMatch>(
              data.matches
            );
  ...
  nextPhase(): Boolean {
    ...
    if (this.getChampionship()!.phase == "Final") {
      ...
      this.semifinalArray.forEach((match: IMatch) => {
      ...
      this.quarterfinalArray.forEach((matches: Array<IMatch>) => {
        matches.forEach((match) => {
          if (match.matchResult1 == null || match.matchResult2 == null) {
            nextPhase = 0;
          }
      ...
      this.roundOf16Array.forEach((matches: Array<IMatch>) => {
        matches.forEach((match) => {
          if (match.matchResult1 == null || match.matchResult2 == null) {
            nextPhase = 0;
          }
      ...
      this.roundOf32Array.forEach((matches: Array<IMatch>) => {
        matches.forEach((match) => {
          if (match.matchResult1 == null || match.matchResult2 == null) {
            nextPhase = 0;
          }
  ...
  getBracketsMatches(): IMatch[] | undefined {
    return this.bracketsMatches;
  }

  setBracketsMatchesEmpty(): void {
    this.bracketsMatches = undefined;
  }
  ...
  getErrorsJoinChampionship(): string | null {
  ...
  private checkCreateFormErrors(errors: any): void {
    if (errors.championshipNameNull)
      this.setCreateErrors(errors.championshipNameNull);
    if (errors.championshipNameMinLength)
      this.setCreateErrors(errors.championshipNameMinLength);
    if (errors.championshipNameMaxLength)
      this.setCreateErrors(errors.championshipNameMaxLength);
    if (errors.userCreatorNull) this.setCreateErrors(errors.userCreatorNull);
    if (errors.userCreatorIncorrect)
      this.setCreateErrors(errors.userCreatorIncorrect);
    if (errors.numMaxTeamsIncorrect)
      this.setCreateErrors(errors.numMaxTeamsIncorrect);
    ...
    if (errors.descriptionMinLength)
      this.setCreateErrors(errors.descriptionMinLength);
    if (errors.descriptionMaxLength)
      this.setCreateErrors(errors.descriptionMaxLength);
    if (errors.startInscriptionNull)
      this.setCreateErrors(errors.startInscriptionNull);
    if (errors.startInscriptionIncorrect)
      this.setCreateErrors(errors.startInscriptionIncorrect);
    if (errors.endInscriptionNull)
      this.setCreateErrors(errors.endInscriptionNull);
    if (errors.endInscriptionIncorrect)
      this.setCreateErrors(errors.endInscriptionIncorrect);
    if (errors.startChampionshipNull)
      this.setCreateErrors(errors.startChampionshipNull);
    if (errors.startChampionshipIncorrect)
      this.setCreateErrors(errors.startChampionshipIncorrect);
    if (errors.championshipLocationNull)
      this.setCreateErrors(errors.championshipLocationNull);
    if (errors.championshipLocationMaxLength)
      this.setCreateErrors(errors.championshipLocationMaxLength);
    if (errors.championshipLocationMinLength)
      this.setCreateErrors(errors.championshipLocationMinLength);
