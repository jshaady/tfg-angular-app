...
export class UserService {
  ...
  private userLoginErrors: string | null = null;
  private userSignUpError: Array<string> = [];
  private usersList: IUser[] = [];
  private emailNotConfirmed: string = "";
  private navLinksUsers: any = [];

  private userUpdateErrors: any = [];
  private isSaving: Boolean = false;

  private dataSource: MatTableDataSource<IUser> | undefined;
  private userListPageOptions: any = {
    previousPageIndex: 0,
    pageIndex: 0,
    pageSize: 25,
    length: 0,
  };
  private searchUserNameOrPassword: string | null = null;

  private statsDataSource: MatTableDataSource<IMatch> | null = null;
  private userStatsPageOptions: any = {
    previousPageIndex: 0,
    pageIndex: 0,
    pageSize: 25,
    length: 0,
  };
  private userStats: IStats | undefined;

  private eventsInfo: any = [
    { label: "leagues", open: false },
    { label: "tournaments", open: false },
    { label: "meets", open: false },
  ];

  private tournamentEventsPage: number = 0;
  private tournamentEvents: IChampionship[] = [];
  private moreTournamentEvents: Boolean = true;

  private leagueEventsPage: number = 0;
  private leagueEvents: IChampionship[] = [];
  private moreLeagueEvents: Boolean = true;

  private meetsEventsPage: number = 0;
  private meetEvents: IMeet[] = [];
  ...
  resendEmail(): void {
    ...
    this.http
      .get<ISnackBar>(this.BaseURL + "/resend/email", { params })
      .subscribe(
        (data) => {
          this.isSaving = false;
  ...
  searchUser(username: string): void {
    ...
    this.http.get<IUser>(this.BaseURL + "/search", { params }).subscribe(
      (data: IUser) => {
        this.user = data;
        this.navLinksUsers = [
          { label: "DESCRIPTION", path: "/user/" + this.user.username },
  ...
  searchUsersList(): void {
    let params = new HttpParams()
      .set("previousPageIndex", this.getUserListPageOptions().previousPageIndex)
  ...
  searchStats(sport: string): void {
    ...
    let params = new HttpParams()
      .set("username", this.getUser()!.username)
      .append(
        "previousPageIndex",
        this.getUserStatsPageOptions().previousPageIndex
      )
  ...
  events(type: string): void {
    const params = new HttpParams()
      .set(
        "page",
        type === "League"
          ? this.getLeagueEventsPage().toString()
          : type === "Tournament"
          ? this.getTournamentEventsPage().toString()
    ...
    this.http.get<any>(this.BaseURL + "/events", { params }).subscribe(
      (data: any) => {
        if (type === "meets") {
          ...
          data.length < 2
            ? (this.moreMeetEvents = false)
            : (this.moreMeetEvents = true);
        } else if (type === "League") {
          ...
          data.length < 2
            ? (this.moreLeagueEvents = false)
            : (this.moreLeagueEvents = true);
          ...
          data.length < 2
            ? (this.moreTournamentEvents = false)
            : (this.moreTournamentEvents = true);
          this.tournamentEvents = data;
        }
      },
      (error: any) => {
  ...
  getUsersList(): IUser[] | null {
    return this.usersList;
  }

  getUserLoginErrors(): string | null {
  ...
  setUserSearchNameOrEmail(nameOrEmail: string | null): any {
  ...
  private checkSignUpFormErrors(errors: any): void {
    ...
    if (errors.errors.userNameNull)
      this.setErrorSignUp(errors.errors.userNameNull);
    if (errors.errors.userNameMinLength)
      this.setErrorSignUp(errors.errors.userNameMinLength);
    if (errors.errors.userNameMaxLength)
      this.setErrorSignUp(errors.errors.userNameMaxLength);
    if (errors.errors.emailIncorrect)
      this.setErrorSignUp(errors.errors.emailIncorrect);
    if (errors.errors.userEmailMaxLength)
      this.setErrorSignUp(errors.errors.userEmailMaxLength);
    if (errors.errors.birthdateNull)
      this.setErrorSignUp(errors.errors.birthdateNull);
    if (errors.errors.birthdateIncorrect)
      this.setErrorSignUp(errors.errors.birthdateIncorrect);
    if (errors.errors.birthdate18orLessYearsOld)
      this.setErrorSignUp(errors.errors.birthdate18orLessYearsOld);
    if (errors.errors.joinDateNull)
      this.setErrorSignUp(errors.errors.birthdateNull);
    if (errors.errors.joinDateIncorrect)
      this.setErrorSignUp(errors.errors.birthdateIncorrect);
    if (errors.errors.joinDateIncorrect)
      this.setErrorSignUp(errors.errors.birthdateIncorrect);
  }
  ...
  private checkUpdateFormErrors(errors: any): void {
    if (errors.errors.nameMaxLength)
      this.setUserUpdateErrors(errors.errors.nameMaxLength);
    if (errors.errors.nameMinLength)
      this.setUserUpdateErrors(errors.errors.nameMinLength);
    if (errors.errors.userSurnameMaxLength)
      this.setUserUpdateErrors(errors.errors.userSurnameMaxLength);
    if (errors.errors.userSurnameMinLength)
      this.setUserUpdateErrors(errors.errors.userSurnameMinLength);
    if (errors.errors.userCountryMaxLength)
      this.setUserUpdateErrors(errors.errors.userCountryMaxLength);
    if (errors.errors.userCountryMinLength)
      this.setUserUpdateErrors(errors.errors.userCountryMinLength);
    if (errors.errors.userLocationMaxLength)
      this.setUserUpdateErrors(errors.errors.userLocationMaxLength);
    if (errors.errors.userLocationMinLength)
      this.setUserUpdateErrors(errors.errors.userLocationMinLength);
    if (errors.errors.userAddressMaxLength)
      this.setUserUpdateErrors(errors.errors.userAddressMaxLength);
    if (errors.errors.userAddressMinLength)
      this.setUserUpdateErrors(errors.errors.userAddressMinLength);
    if (errors.errors.userPhoneNotNumber)
      this.setUserUpdateErrors(errors.errors.userPhoneNotNumber);
    if (errors.errors.userPhoneMinLength)
      this.setUserUpdateErrors(errors.errors.userPhoneMinLength);
    if (errors.errors.userPhoneMaxLength)
      this.setUserUpdateErrors(errors.errors.userPhoneMaxLength);
    if (errors.errors.birthdateNull)
      this.setUserUpdateErrors(errors.errors.birthdateNull);
    if (errors.errors.birthdateIncorrect)
      this.setUserUpdateErrors(errors.errors.birthdateIncorrect);
    if (errors.errors.birthdate18orLessYearsOld)
      this.setErrorSignUp(errors.errors.birthdate18orLessYearsOld);
    if (errors.errors.descriptionMaxLength)
      this.setUserUpdateErrors(errors.errors.descriptionMaxLength);
    if (errors.errors.descriptionMinLength)
      this.setUserUpdateErrors(errors.errors.descriptionMinLength);
  }
