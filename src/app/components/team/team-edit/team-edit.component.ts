...
export class TeamEditComponent implements OnInit {
  ...
  formModel = this.fb.group({
    Description: [[Validators.required, Validators.minLength(5), Validators.maxLength(512)]],
    Location: [[Validators.required, Validators.minLength(1), Validators.maxLength(255)]]
  });

  formModelAvatar = this.fb.group({
  ...
  uploadFile() {
    if (this.formModelAvatar.get('Avatar') != null) {
      this.teamService.setIsSaving(true);
      this.teamService.updateTeamAvatar(this.formModelAvatar.get('Avatar').value)
  ...
  getTeamAvatar() {
    if (this.teamService.getTeam() && this.teamService.getTeam().imageType !== null && this.teamService.getTeam().imageBase64 !== null) {
  ...
  checkTeamCreator(): Boolean {
    if (this.teamService.getTeam().userLeader === this.userService.getLoggeduser().username) {
      return true;
    }
    else {
      this.teamService.makeSnackBar('You not the team leader, cant edit the info', 'error');
