...
export class TeamViewComponent implements OnInit {
  ...
  searchCode(): void {
    ...
    setTimeout( () => {
      if (this.teamService.getUuid() != null) {
        ...
        this.teamService.makeSnackBar('Team code copied to clipboard', 'success');
      }
    }, 300);
  }
  ...
  canJoin(): Boolean {
    if (this.getTeam().isPrivate === 0 && this.userService.loggedIn()) {
      let isInTeam = true;
      this.getTeam().usersInTeam.forEach(user => {
  ...
  alreadyJoined(): Boolean {
    if (this.userService.getLoggeduser() && 
        !this.isLeader() &&
        this.getTeam().usersInTeam.find(element => element.username === this.userService.getLoggeduser().username)){
      return true;
    }
    else {
      return false;
